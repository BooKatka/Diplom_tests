...
Цель проекта: Тестирования сайта КиноПоиск (https://www.kinopoisk.ru/) с использованием  Автоматизации UI и API, Python, Pytest, Selenium и Allure. .

## Для чего этот проект

Создана комплексная инфраструктура автоматизированного тестирования, охватывающая интерфейс сайта и его API. Благодаря выстроенному процессу тестирования, достигается высокая надежность работы проекта и быстрое выявление багов.
...
├── tests/
│   ├── Diplom_test_ui.py          # UI-тесты для веб-интерфейса Кинопоиска
│   └── Diplon_test_api.py         # API-тесты для Kinopoisk API
...
```bash
pytest tests/Diplom_test_ui.py --alluredir=allure-results
```

**Запуск только API-тестов**:
```bash
pytest tests/Diplom_test_api.py --alluredir=allure-results
```
...
Дипломная работа выполнена студентом курса автоматизации тестирования, поток 108.3.

**ФИО**: <Бурученко Екатерина>
**Год**: 2025